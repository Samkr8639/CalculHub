...
@Component({
  selector: 'app-car-loan-emi-calculator',
  imports: [CommonModule, DecimalPipe, RouterLink],
  templateUrl: './car-loan-emi-calculator.html',
  styleUrls: ['./car-loan-emi-calculator.css'],
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class CarLoanEmiCalculatorComponent {
  private platformId = inject(PLATFORM_ID);

  loanAmount = signal<number | null>(800000);
  interestRate = signal<number | null>(9.5);
  ...
  private generateChartData() {
    return {
      ...
      data: [this.loanAmount() ?? 0, this.totalInterest() ?? 0],
    };
  }
  ...
  private createChart() {
    ...
    const chartData: ChartData = {
      labels,
      datasets: [{ data, backgroundColor: ['#36a2eb', '#ff6384'], borderWidth: 1 }],
    };
